	...
	private class processSendingTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... tmp) {
			try {
				...
				nameValuePairs.add(new BasicNameValuePair("jmeno", et_desc.getText().toString()));
				nameValuePairs.add(new BasicNameValuePair("lat",et_location.getText().toString()));
			//	nameValuePairs.add(new BasicNameValuePair("lon", Double.toString(camera.getLongitude())));
